...
export default function CollegeDashboard() {
  ...
  // Calculate maximum salary from jobs data
  const maxSalary = jobsData.reduce((max, job) => {
    return job.salaryRange?.max > max ? job.salaryRange.max : max;
  }, 0);

  const statsData = [
    {
      title: "Tests Taken",
      value: assessmentData.length.toString(),
      bgColor: "bg-[#E6F5F9]",
    ...
    {
      title: "Best CTC Offered So Far",
      value: maxSalary ? `₹${maxSalary}L` : "₹0L",
      bgColor: "bg-[#EEF6FD]",
      textColor: "text-[#219CAE]",
    },
    {
      title: "Students",
      value: collegeDetails?.students?.length?.toString() || "0",
      bgColor: "bg-[#F9F0FF]",
      textColor: "text-[#219CAE]",
    },
  ];

  // Transform assessment data for chart - show ALL assessments
  const chartData = assessmentData
    ...
    .map((assessment: any) => {
      ...
      return {
        ...
        value: appearedCount, // Y-axis: appeared students
        invitedCount,
        ...
        participationRate:
          invitedCount > 0
            ? Math.round((appearedCount / invitedCount) * 100)
            : 0,
      };
    });
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const collegeResponse = await axios.get(
          `${process.env.NEXT_PUBLIC_QALIO_BACKEND_URL}/college/getSavedDetails`,
          {
            headers: {
        ...
        setCollegeDetails(collegeResponse.data.college);
        ...
        const assessmentData = assessmentResponse.data.assessments;
        const filteredAssessments = assessmentData.filter(
          (d: { startDate: string }) =>
            new Date(d.startDate).getTime() <= Date.now()
        );

        const processedAssessments = filteredAssessments.map((item: any) => ({
          ...item,
  ...
  return (
    <>
      ...
      <OverviewSection
        stats={statsData}
        chartData={chartData}
        chartTitle="Assessment Analytics"
      />
      <h2 className="text-xl font-semibold text-gray-800 mb-6 mt-6 font-[jost]">
        Result Status
      </h2>
