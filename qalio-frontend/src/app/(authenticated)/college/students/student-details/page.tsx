...
import { PublishStudentButton } from "./PublishStudentButton";
import CopyPublicLinkButton from "./CopyPublicLinkButton";
import { useAuthStore } from "@/stores/auth-store";
import ProfilePageSkeleton from "@/app/(authenticated)/student/profile/profileLoader";
...
export default function ProfilePage() {
  ...
  // Get studentId from URL params, if not present, fetch current user's profile
    ...
    try {
      ...
      if (token) {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_QALIO_BACKEND_URL}/student/forcollege/${studentId}`,
          {
            method: "GET",
            headers: {
      ...
      // If no token or failed above, try public route
      if (!studentData) {
        const res = await fetch(
  ...
  const handleEditProfile = (section?: string) => {
    // Only allow editing if it's not a public view and it's the current user's profile
    if (isPublicView || studentId) {
      ...
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-gray-500 mt-2">
            {studentId
              ? "Student profile not found or not public"
  ...
  const canEdit = !isPublicView && !studentId;
    ...
    <div>
      ...
      <div className="flex-1">
        ...
        <div className="flex items-center justify-between mt-2">
          <BackHeader title={`${studentProfile.userId.name}`} />
        </div>
        <div className="bg-white rounded-lg shadow-md p-4 md:p-6 mb-6 relative">
          <div className="flex justify-between items-start mb-4">
            <div></div> {/* Empty div for flex spacing */}
            <div className="flex gap-2">
              {/* Show publish/copy buttons only in private view for specific student */}
              {!isPublicView && studentId && (
                <>
                  <PublishStudentButton studentId={studentProfile._id} />
                  <CopyPublicLinkButton studentId={studentProfile._id} />
                </>
              )}

              {/* Show edit buttons only if user can edit */}
              {canEdit && (
                <>
                  <Dialog
