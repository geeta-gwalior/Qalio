...
export default function Results() {
  ...
  const chartData = assessmentData
    ...
    .slice(-10)
    .reverse() // Show last 10 assessments
    .map((assessment: any) => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      try {
        ...
        if (!res.data.assessments || res.data.assessments.length === 0) {
          ...
          const transformedData = res.data.assessments.map(
            (assessment: any) => {
              ...
              const resultPolicy =
                assessment?.assessmentInfo?.config?.resultPolicy;
              // console.log(resultPolicy);
              const evaluatedStatus =
                assessment.attempts[0]?.evaluatedStatus?.toLowerCase();

              // console.log(evaluatedStatus);
              // Check for manual resultPolicy and pending evaluation
              const displayMarks =
              ...
              return {
                ...
                highestAttemptMarks: displayMarks,
                ...
                evaluatedStatus: assessment.attempts[0]?.evaluatedStatus,
              };
            }
          );
        ...
        // console.log("Error from results:", error); // Handle 404 specifically for first-time users (if backend still returns 404)
  ...
  const paginatedData = assessmentData
    .reverse()
