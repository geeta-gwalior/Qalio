...
type MCQQuestion = {
  ...
  totalMarks: number;
  questionTopic: string; // Added questionTopic field
};
...
export default function AddMCQQuestion() {
  ...
  const [currentQuestion, setCurrentQuestion] = useState<MCQQuestion>({
    ...
    totalMarks: 1,
    questionTopic: "", // Added questionTopic to initial state
  });
  ...
  const handleSaveQuestion = async () => {
    ...
    if (!currentQuestion.questionTopic.trim()) {
      toast("Please enter a topic for the question");
      return;
    }
    ...
    try {
      ...
      await axios.post("/api/questions", {
        ...
        questionTopic: currentQuestion.questionTopic, // Added questionTopic to API payload
      });
  ...
  const handleAddNextQuestion = async () => {
    ...
    try {
      ...
      setCurrentQuestion({
        ...
        questionLevel: currentQuestion.questionLevel,
        totalMarks: currentQuestion.totalMarks,
        questionTopic: "", // Reset questionTopic for new question
      });
    } catch (error) {
      console.error("Error saving question:", error);
      toast.error("Failed to save question. Please try again.");
      ...
      <div className="w-full   ">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 bg-white rounded-lg shadow-sm">
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <Input
                  ...
                  placeholder="Enter topic (e.g., binary search, DSA)"
                  className="w-full"
                />
              </div>
              <div className="mb-4">
          ...
          <div>
            ...
            <RadioGroup
              value={`${correctOptionIndex !== -1 ? correctOptionIndex : 0}`}
              onValueChange={(value) =>
