...
export default function QalioLogo({
  ...
  return (
    <div className={`inline-flex items-center gap-2.5 select-none ${containerPadding[variant]} ${className}`}>
      {/* Icon Badge */}
      <div
        className={`${iconSizes[size]} bg-gradient-to-br from-[#2AABDE] to-[#219CAE] flex items-center justify-center text-white font-black shadow-md transition-transform hover:scale-105`}
      >
      ...
      {/* Brand Text */}
      {showText && (
        <span className={`${textSizes[size]} font-black tracking-tight text-[#1A2B3B] font-[jost]`}>
          Qalio
        </span>
      )}
    </div>
